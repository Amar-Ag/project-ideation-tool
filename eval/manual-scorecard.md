...
def evaluate_session(client: Groq, session_id: str, session_data: dict) -> dict:
    ...
    scored_count = sum(
        1 for c in result["criteria"].values()
    ...
    if any_dq:
        ...
        conv_score = sum(
            c.get("score", 0) for cid, c in result["criteria"].items()
            if c.get("score") is not None
        )
        conv_max = scored_count * 3
        result["verdict"] = f"Incomplete ({conv_score}/{conv_max} on conversation quality)"
    elif total >= 28:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ideation sessions with LLM-as-judge")
    parser.add_argument("--input", required=True, help="Path to CSV export from Supabase")
    parser.add_argument("--output", default="eval/results.json", help="Path for JSON results")
    parser.add_argument("--markdown", default="eval/results.md", help="Path for markdown report")
    parser.add_argument("--session", default=None, help="Evaluate a single session ID (prefix match)")
    parser.add_argument("--rerun", action="store_true", help="Re-evaluate sessions even if already scored")
    ...
    # Print summary
    print("\n" + "=" * 60)
    print(f"This run: {len(new_results)} new | {len(existing_results)} existing | {len(all_results)} total")
    print("=" * 60)
